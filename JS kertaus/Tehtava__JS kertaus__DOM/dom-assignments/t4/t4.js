...
// list for putting address, distance and name
const calculatedLocations = [];

// get my location
navigator.geolocation.getCurrentPosition(function(position) {
  console.log(position);
  //my latitude position
  const latitude = position.coords.latitude;
  //my longitude position
  const longitude = position.coords.longitude;
  // getting each resturant location and calculate distance put distance address and name to new list
  restaurants.forEach(item => {

    let a = (Math.cos(latitude * Math.PI / 180)) * (Math.cos(item.location.coordinates[1] * Math.PI / 180)) * (Math.cos(longitude * Math.PI / 180)) * (Math.cos(item.location.coordinates[0] * Math.PI / 180));

    let b = (Math.cos(latitude * Math.PI / 180)) * (Math.sin(longitude * Math.PI / 180))  * (Math.cos(item.location.coordinates[1] * Math.PI / 180)) * (Math.sin(item.location.coordinates[0] * Math.PI / 180));

    let c = (Math.sin(latitude * Math.PI/180) * Math.sin(item.location.coordinates[1] * Math.PI / 180));

    let r = 6370;

    let d = Math.acos(a+b+c) * r;



    calculatedLocations.push({ name: item.name, address: item.address, distance: d });

       });
       console.log(calculatedLocations)

       // Sort the calculatedLocations array by distance in ascending order
       const sortedLocations = calculatedLocations.sort((a, b) => a.distance - b.distance);


       sortedLocations.forEach(i => {
        ...
        tdAddress.textContent = i.address;
...
//   restaurants.forEach(item => {
//     var a = Math.cos(position.coords.latitude * Math.PI / 180)
...
//     var tbAddress = document.createElement("td");

//     tbName.textContent = name;
//     tbAddress.textContent = address;

//     tr.appendChild(tbName);
//     tr.appendChild(tbAddress);
//     mylist.appendChild(tr);
//     // var latitude = item.location.coordinates[1];
//     // var longitude = item.location.coordinates[0];

//     // var tr = document.createElement("tr")
//     // var tbLatitude = document.createElement("td");
//     // var tbLongitude = document.createElement("td");

//     // tbLatitude.textContent = latitude;
//     // tbLongitude.textContent = longitude;

//     // tr.appendChild(tbLatitude);
//     // tr.appendChild(tbLongitude);
//     // mylist.appendChild(tr);
